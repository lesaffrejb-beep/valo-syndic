...
export function DownloadPptxButton({ result }: DownloadPptxButtonProps) {
    ...
    const generatePptx = async () => {
        ...
        try {
            ...
            // Financing table
            const tableData = [
                ["Poste", "Montant Global", "Par Lot"],
                ["Coût total HT", formatCurrency(result.financing.totalCostHT), formatCurrency(result.financing.costPerUnit)],
                ["MaPrimeRénov'", `-${formatCurrency(result.financing.mprAmount)}`, `-${formatCurrency(result.financing.mprAmount / result.input.numberOfUnits)}`],
                ["Éco-PTZ (prêt 0%)", formatCurrency(result.financing.ecoPtzAmount), formatCurrency(result.financing.ecoPtzAmount / result.input.numberOfUnits)],
                ["Reste à charge", formatCurrency(result.financing.remainingCost), formatCurrency(result.financing.remainingCostPerUnit)],
            ];
